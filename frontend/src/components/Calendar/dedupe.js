...
async function runDedupe() {
  ...
  try {
    ...
    // Compares only the last 10 digits and trims spaces to ensure matches
    const clientRes = await db.query(`
      DELETE FROM clients a USING clients b
      WHERE a.id > b.id
        AND RIGHT(regexp_replace(TRIM(a.phone), '\\D', '', 'g'), 10) = RIGHT(regexp_replace(TRIM(b.phone), '\\D', '', 'g'), 10)
    `);
